...
type Doc = { id: string; name: string; content?: string; extracted_entities?: Record<string, unknown> }
...
export function SplitView({ document, onSelectDocument }: SplitViewProps) {
  ...
  useEffect(() => {
    fetch("/api/v1/operations/feed")
      .then((r) => (r.ok ? r.json() : null))
      .then((json) => {
        const docs: Doc[] = (json?.data ?? []).map((d: { id: string; name: string; content?: string | null; extracted_entities?: Record<string, unknown> | null }) => ({
          id: d.id,
          name: d.name,
          content: d.content ?? undefined,
          extracted_entities: d.extracted_entities ?? undefined,
        }))
  ...
  useEffect(() => {
    chatMessages.length > 1 && chatScrollRef.current?.scrollTo({ top: chatScrollRef.current.scrollHeight, behavior: "smooth" })
  }, [chatMessages])
  ...
  const picker = (
    <div className="p-3 border-b border-border shrink-0">
      <Select value={document?.id ?? ""} onValueChange={handlePick} disabled={docsLoading || availableDocs.length === 0}>
        <SelectTrigger className="text-sm">
          <SelectValue placeholder={docsLoading ? "Loading documents..." : availableDocs.length === 0 ? "No documents in feed yet" : "Select a document to view"} />
        </SelectTrigger>
        <SelectContent>
          {availableDocs.map((d) => (
    ...
    return (
      <div className="border border-border rounded-lg overflow-hidden">
        ...
        <div className="flex items-center justify-center h-64 text-muted-foreground text-sm">
          {availableDocs.length === 0 && !docsLoading ? "No documents yet — add one from the Document Feed tab." : "Select a document above to view it"}
        </div>
      </div>
    )
  }
