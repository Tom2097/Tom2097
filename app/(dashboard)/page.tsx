  ...
  function Err({ msg, detail, uid, oid }: { msg: string; detail?: string; uid?: string; oid?: string }) {
    return (
      <div style={{background:"#111",color:"#f44",padding:40,fontFamily:"monospace",minHeight:"100vh"}}>
        <h1 style={{fontSize:24,marginBottom:16}}>{t("dashboard.error.authError")} {msg}</h1>
        {detail && <p>{t("dashboard.error.detail")} {detail}</p>}
        {uid && <p>{t("dashboard.error.user")} {uid}</p>}
        {oid && <p>{t("dashboard.error.org")} {oid}</p>}
        <div style={{marginTop:20}}>
          <a href="/login-check" style={{color:"#0ff"}}>{t("dashboard.error.goToLoginCheck")}</a>
          {' | '}
          <a href="/auth/login" style={{color:"#0ff"}}>{t("dashboard.error.backToLogin")}</a>
        </div>
      </div>
    )
  }
