...
import { summarizeDocument } from "@/lib/operational/understanding"
...
export async function POST(request: Request) {
  try {
    ...
    const result = await summarizeDocument(orgId, documentId, content)
    if (!result) return NextResponse.json({ error: "Summarization failed" }, { status: 500 })
